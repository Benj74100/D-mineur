#########################################
### Info301, TP3 : démineur en Python ###
...
# Question 1
# Si on donne un nombre négatif elle ne s'exécute pas


# Question 2
# La fonction renvoie le nombre de fois que la lettre A minuscule apparaît dans le texte
...
def genere_plateau(largeur, hauteur, probabilite_mine):
    ...
    for i in range(hauteur):
        ...
        for j in range(largeur):
            ligne.append({"mine": random.random() < probabilite_mine,
                          "etat": INCONNU})
...
def cases_voisines(plateau, i, j):
    ...
    """
    res = []
    for i, j in [(i-1,j-1), (i,j-1), (i+1,j-1), (i+1,j), (i+1,j+1), (i,j+1), (i-1,j+1), (i-1,j)]:
        if dans_plateau(plateau, i, j):
            res.append((i, j))
    return res
...
def compte_mines_voisines(plateau, i, j):
    ...
    """
    nbmine = 0
    for i, j in cases_voisines(plateau, i, j):
        if plateau[i][j]["mine"]:
            nbmine += 1
    return nbmine
...
def composante_connexe(plateau, i, j):
    ...
    #nombre de mines autours de la case (i,j)
    if plateau[i][j]["etat"] == INCONNU:
        c = compte_mines_voisines(plateau, i, j)
        plateau[i][j]["etat"] = c
        if c == 0:
            for i, j in cases_voisines(plateau, i,j):
...
def gagne(plateau):
    ...
    for ligne in plateau:
        for case in ligne:
            if case["mine"] and case["etat"] != DRAPEAU:
                return False
            if not case["mine"] and case["etat"] >= 0:
                return False
    return True
...
def __action_clic_d(clic):
    ...
    if plateau_courant[i][j]["etat"] == INCONNU:
        plateau_courant[i][j]["etat"] = DRAPEAU
    elif plateau_courant[i][j]["etat"] == DRAPEAU:
        plateau_courant[i][j]["etat"] = QUESTION
    elif plateau_courant[i][j]["etat"] == QUESTION:
        plateau_courant[i][j]["etat"] = INCONNU
...
grille.bind("<Button-3>", __action_clic_d)
